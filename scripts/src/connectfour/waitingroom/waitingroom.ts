...
function CheckInput() {
	...
	const rowInput = document.getElementById("gameRowCount") as HTMLInputElement;
...
async function UserJoinWaitingPool() {
	...
	const rows = (document.getElementById("gameRowCount") as HTMLInputElement)
		.value;

	const columns = (
		document.getElementById("gameColumnCount") as HTMLInputElement
	).value;

	const connectionSize = Number(
		(document.getElementById("gameConnectionSize") as HTMLInputElement).value
	);
...
async function LeaveWaitingPool() {
	if (localStorage.getItem("candidateCheckInterval") !== null) {
		...
		if (response.ok) {
			...
			(document.querySelector("#gameRowCount") as HTMLInputElement).disabled =
				false;

			(
				document.querySelector("#gameColumnCount") as HTMLInputElement
			).disabled = false;

			(
				document.querySelector("#gameConnectionSize") as HTMLInputElement
			).disabled = false;
		} else {
			CreateToastMsg(
