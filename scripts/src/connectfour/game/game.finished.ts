 ...
 * @param {*} game
 */
async function finishGame(game: GameModel) {
	...
	const dialog = document.getElementById("gameDialog") as HTMLDialogElement;
 ...
 */
function highlightWinningConnection(winningConnections: ConnectionModel[]) {
	...
	for (let i = 0; i < winningConnections.length; i++) {
		...
		if (rowDiff == 0) {
			...
			for (
				...
				table.rows[i].cells[winningConnection.from.column].classList.add(
					"winning"
				);
			}
		} else {
...
async function updateLeaderboardWithGame(gameId: string) {
