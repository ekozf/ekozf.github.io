...
function BuildCategoryItemCards(familyItems) {
    const cards = [];
...
function BuildItemCards(familyItems) {
    const cards = [];
    for (let i = 0; i < familyItems.length; i++) {
        const item = familyItems[i];
        const wrapper = document.createElement("div");
        wrapper.classList.add("item-card", "mx-3", "mb-4", "rounded-3", "p-3", "d-flex", "justify-content-between", "shadow");
